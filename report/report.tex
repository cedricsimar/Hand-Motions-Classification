...
\begin{titlepage}
	...
	{\LARGE\bfseries INFO-F-422 \\ Statistical foundations of Machine Learning \par
		\vspace{3ex}}
	{\itshape\Large Classifying Sample Covariance Matrices \\ by considering the geometry of Riemannian manifolds: \\ Application to Hand Motions Recognition \par}
	\vfill
	\makeatletter
	{\large \@author\par}
	\vfill
...
Let's make a necessarily short descrption of what Riemannian manifolds are. They have a quite heavy
formalism, and rather simple ideas can become confusing if we stick to much to formulas. However, let's introduce them
properly by defining what topological spaces, manifolds, differential manifolds and Riemannian manifolds are.
...
\section{Logarithmic and exponential maps}

Riemannian manifolds are smooth manifolds (differentiable $C^k$-manifolds for which derivatives of all orders exist up to $k$)
for which a scalar product is defined in the tangent space at any point on them. Given a manifold $\mathcal{M}$ and a point $p$,
the tangent space is commonly denoted by $T_p \mathcal{M}$.
